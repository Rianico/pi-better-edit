...
const ompSeqCalls = edits.map(
  ([, , newText], index) => `[${ompPath}#${ompTag}]\n${ompHunk(index + 1, newText)}`,
);
const ompBatchCall = `[${ompPath}#${ompTag}]\n${edits
  .map(([, , newText], index) => ompHunk(index + 1, newText))
  .join("\n")}\n`;
const ompSeqTokens = ompSeqCalls.reduce((total, call) => total + tokenCount(call), 0);
const ompBatchTokens = tokenCount(ompBatchCall);
const strReplaceTokens = strReplaceCalls.reduce((total, call) => total + tokenCount(call), 0);
