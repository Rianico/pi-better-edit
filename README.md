...
- **Single-edit pipeline.** The tool's `execute` function validates and applies exactly one edit per call: `hash_range_inclusive` and `content_lines` stay at the top level through the whole pipeline. The `normReq` function in `replace-normalize.ts` applies only cosmetic normalization (`file_path` → `path`, JSON-string `content_lines` parsing), so any code path that normalizes input (e.g. `compPreview`) sees the same shape as the schema. The `changes` array dialect is not accepted from the model — `[E_BAD_SHAPE]` rejects it before any file I/O.
