...
function resolveServedRange(input: ComputeDriftInput): {
  ...
  if (!range) throw new Error("[E_BAD_SHAPE] computeDrift requires range or intervals");
...
function resolveIntervals(input: ComputeDriftInput): Array<{ from: number; to: number; delta: number }> {
...
function deltaBefore(p: number, ranges: Array<{ from: number; to: number; delta: number }>): number {
  let d = 0;
...
export function computeDrift(
  ...
  if (useIntervals) {
    ({ total, unshown: initialUnshown, anyNotReported, driftedPositions } =
      collectDriftedIntervals(input, resultHashSet, currentPosOfHash, intervals));
  } else {
    const { rangeFrom, rangeTo } = resolveServedRange(input);
    ({ total, unshown: initialUnshown, anyNotReported, driftedPositions } = collectDrifted(
      input,
      resultHashSet,
      currentPosOfHash,
      rangeFrom,
