...
function assertItem(edit: Record<string, unknown>): void {
  ...
  if ("remove_from" in edit && typeof edit.remove_from !== "string") {
    throw new Error(`[E_BAD_PAYLOAD] Field "remove_from" must be an anchor string (3-char hash).`);
  }
  if ("remove_to" in edit && typeof edit.remove_to !== "string") {
    throw new Error(`[E_BAD_PAYLOAD] Field "remove_to" must be an anchor string (3-char hash).`);
  }
