...
export const EDIT_GUIDELINES: string[] = [
  ...
  "edit: a `[MODEL]` line in `content` is your retry instruction — follow it from the message alone, unless it states No action is required (soft hint, bytes already written as-is); a `[MODEL]` line that presents rows as a fresh read (`Current range (fresh read):`) is not a blind retry — decide from those rows; a dimmed `[USER]` line in `details` is human info, never your error.",
