...
describe("diff rows serve chained edits", () => {
  it("serves replace diff rows so follow-up edits anchor on them without a read", async () => {
    await withTempFile("sample.ts", "l1\nl2\nl3\nl4\nl5\n", async ({ cwd }) => {
      ...
      const l2Ref = extractHash(readText.split("\n").find((l) => l.includes("│l2"))!);

      const editResult = await editTool.execute(
        "e1",
        { path: "sample.ts", remove_from: l2Ref, remove_to: l2Ref, replacement_text: "X" },
        undefined,
        undefined,
        ctx,
      );
      ...
      const chained = await editTool.execute(
        "e2",
        { path: "sample.ts", remove_from: l1Ref, remove_to: xRef, replacement_text: "A\nB" },
        undefined,
        undefined,
        ctx,
      );
      expect(getText(chained)).toContain("Successfully replaced");
      expect(await readFile(join(cwd, "sample.ts"), "utf-8")).toBe("A\nB\nl3\nl4\nl5\n");
    });
  });
  ...
  it("rejects a follow-up anchored on diff rows when the handler never ran (no serve)", async () => {
    await withTempFile("sample.ts", "l1\nl2\nl3\nl4\nl5\n", async ({ cwd }) => {
      ...
      expect(await readFile(join(cwd, "sample.ts"), "utf-8")).toBe("l1\nX\nl3\nl4\nl5\n");
    });
  });
  ...
  it("recovers in one reject-and-serve roundtrip when auto-read is disabled", async () => {
    await withTempFile("sample.ts", "l1\nl2\nl3\nl4\nl5\n", async ({ cwd }) => {
      ...
      const { handlers, getTool } = makeSeamPi();
      const sessionStart = handlers.get("session_start") as (event: unknown, ctx: unknown) => Promise<unknown>;
      await sessionStart({}, { getActiveTools: () => [], setActiveTools: () => {}, ui: { notify() {} } });
      ...
      const l1Ref = extractHash(text.split("\n").find((l) => l.includes("│l1"))!);
      const l2Ref = extractHash(text.split("\n").find((l) => l.includes("│l2"))!);
      ...
      try {
        await editTool.execute(
          "e2",
          { path: "sample.ts", remove_from: l1Ref, remove_to: l3Ref, replacement_text: "A\nB\nC" },
          undefined,
          undefined,
          ctx,
        );
      ...
      const retry = await editTool.execute(
        "e3",
        { path: "sample.ts", remove_from: retryFrom, remove_to: retryTo, replacement_text: "A\nB\nC" },
        undefined,
        undefined,
        ctx,
      );
