...
describe("strict hashline tool loop", () => {
    ...
    await withTempFile("sample.ts", "alpha\nbeta\n", async ({ cwd }) => {
      ...
      await expect(
        editTool.execute(
          ...
          {
            path: "sample.ts",
            changes: [{ hash_range_incl: [betaRef, betaRef], content_lines: ["BETA-AGAIN"] }],
          },
          undefined,
          undefined,
          ctx,
      ...
      await editTool.execute(
        ...
        {
          path: "sample.ts",
          changes: [{ hash_range_incl: [freshRef, freshRef], content_lines: ["BETA-AGAIN"] }],
        },
        undefined,
        undefined,
        ctx,
  ...
  it("seeds content into an empty file via the empty-line hash", async () => {
    await withTempFile("empty.ts", "", async ({ cwd, path }) => {
      ...
      await editTool.execute(
        "e1",
        { path: "empty.ts", changes: [{ hash_range_incl: [emptyHash, emptyHash], content_lines: ["first", "second"] }] },
        undefined,
        undefined,
        ctx,
      );
...
describe("CRLF line ending preservation", () => {
  it("preserves CRLF line endings after edit", async () => {
    await withTempFile("crlf.ts", "alpha\r\nbeta\r\ngamma\r\n", async ({ cwd, path }) => {
      ...
      await editTool.execute(
        "e1",
        { path: "crlf.ts", changes: [{ hash_range_incl: [betaRef, betaRef], content_lines: ["BETA"] }] },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("preserves LF line endings after edit (no CRLF introduced)", async () => {
    await withTempFile("lf.ts", "alpha\nbeta\ngamma\n", async ({ cwd, path }) => {
      ...
      await editTool.execute(
        "e1",
        { path: "lf.ts", changes: [{ hash_range_incl: [betaRef, betaRef], content_lines: ["BETA"] }] },
        undefined,
        undefined,
        ctx,
      );
...
describe("UTF-8 BOM handling", () => {
  it("strips the BOM when reading and does not restore it on write", async () => {
    await withTempFile("bom.ts", "alpha\nbeta\n", async ({ cwd, path }) => {
      ...
      await editTool.execute(
        "e1",
        { path: "bom.ts", changes: [{ hash_range_incl: [betaRef, betaRef], content_lines: ["BETA"] }] },
        undefined,
        undefined,
        ctx,
      );
