...
describe("chained edit anchors", () => {
  it("returns updated anchors in edit result for a single-line replace", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\n", async ({ cwd }) => {
      ...
      const editResult = await editTool.execute(
        "e1",
        { path: "sample.ts", changes: [{ hash_range_inclusive: [betaRef, betaRef], content_lines: ["BETA"] }] },
        undefined,
        undefined,
        ctx,
      );
      ...
      const editResult2 = await editTool.execute(
        "e2",
        { path: "sample.ts", changes: [{ hash_range_inclusive: [freshRef, freshRef], content_lines: ["BETA-CHAINED"] }] },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("omits anchors when post-edit affected span is too large", async () => {
    ...
    await withTempFile("big.ts", fifteenLines, async ({ cwd }) => {
      ...
      const editResult = await editTool.execute(
        ...
        {
          path: "big.ts",
          changes: [{ hash_range_inclusive: [line1Ref, line15Ref], content_lines: newLines }],
        },
        undefined,
        undefined,
        ctx,
  ...
  it("omits anchors when single-line replace expands beyond budget", async () => {
    ...
    await withTempFile("expand.ts", "before\ntarget\nafter\n", async ({ cwd }) => {
      ...
      const editResult = await editTool.execute(
        "e1",
        { path: "expand.ts", changes: [{ hash_range_inclusive: [targetRef, targetRef], content_lines: newLines }] },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("unchanged line anchors from original read remain valid after chained edits", async () => {
    await withTempFile("stale.ts", "alpha\nbeta\n", async ({ cwd }) => {
      ...
      await editTool.execute(
        "e1",
        { path: "stale.ts", changes: [{ hash_range_inclusive: [betaRef, betaRef], content_lines: ["BETA"] }] },
        undefined,
        undefined,
        ctx,
      );
      await expect(
        editTool.execute(
          "e2-stale",
          { path: "stale.ts", changes: [{ hash_range_inclusive: [betaRef, betaRef], content_lines: ["BETA-AGAIN"] }] },
          undefined,
          undefined,
          ctx,
        ),
      ...
      const alphaEdit = await editTool.execute(
        "e3",
        { path: "stale.ts", changes: [{ hash_range_inclusive: [alphaRef, alphaRef], content_lines: ["ALPHA"] }] },
        undefined,
        undefined,
        ctx,
      );
