  ...
  it("echoes the current range as fresh rows; retrying with them applies without read and does not loop", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\n", async ({ cwd, path }) => {
      ...
      const retry = await editTool.execute(
        ...
        {
          path: "sample.ts",
          remove_from: retryFrom, remove_to: retryTo, replacement_text: "X\nY",
        },
        undefined, undefined, ctx,
      );
      ...
      const secondStale = await editTool.execute(
        ...
        {
          path: "sample.ts",
          remove_from: retryFrom, remove_to: retryTo, replacement_text: "Z",
        },
        undefined, undefined, ctx,
      );
  ...
  it("tolerates an out-of-range in-place content change below the range", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\ndelta\n", async ({ cwd, path }) => {
      ...
      const result = await editTool.execute(
        ...
        {
          path: "sample.ts",
          remove_from: alphaRef, remove_to: betaRef, replacement_text: "A\nB",
        },
        undefined, undefined, ctx,
      );
  ...
  it("tolerates a deletion above the range (positional shift)", async () => {
    await withTempFile("sample.ts", "a1\na2\nbeta\ngamma\n", async ({ cwd, path }) => {
      ...
      const result = await editTool.execute(
        ...
        {
          path: "sample.ts",
          remove_from: betaRef, remove_to: gammaRef, replacement_text: "B\nG",
        },
        undefined, undefined, ctx,
      );
      expect(getText(result)).toContain("Successfully replaced");
    ...
    await withTempFile("sample.ts", "alpha\nbeta\n", async ({ cwd, path }) => {
      ...
      try {
        await editTool.execute(
          ...
          {
            path: "sample.ts",
            remove_from: alphaRef, remove_to: betaRef, replacement_text: "X",
          },
          undefined, undefined, ctx,
        );
      } catch (error) {
      ...
      const retry = await editTool.execute(
        ...
        {
          path: "sample.ts",
          remove_from: betaRefFromContext, remove_to: betaRefFromContext, replacement_text: "BETA2",
        },
        undefined, undefined, ctx,
      );
      expect(getText(retry)).toContain("Successfully replaced");
    ...
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\n", async ({ cwd, path }) => {
      ...
      await expect(
        editTool.execute(
          ...
          {
            path: "sample.ts",
            remove_from: hashes[0]!, remove_to: hashes[2]!, replacement_text: "X",
          },
          undefined, undefined, ctx,
        ),
