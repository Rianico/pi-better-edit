...
describe("drift notices for changed served territory outside the edit range", () => {
  ...
  it("undo_last_edit results never carry a drift notice", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\ndelta\n", async ({ cwd, path }) => {
      ...
      const undone = await undoTool.execute("u1", { path: "sample.ts" }, undefined, undefined, ctx);
  ...
  it("shows the before/after lines around the drifted content and serves the context rows for follow-up edits", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\ndelta\n", async ({ cwd, path }) => {
      ...
      expect(notice.indexOf(betaRow!)).toBeLessThan(notice.indexOf(gammaRow!));
      expect(notice.indexOf(gammaRow!)).toBeLessThan(notice.indexOf(deltaRow!));
