...
describeGate("EVAL comparison battery", () => {
  it("runs the battery", async () => {
    ...
    await withTempFile("b1.ts", "aaa\nbbb\nccc\n", async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b1.ts", remove_from: anchor, remove_to: anchor, replacement_text: "BBB" }, ctx);
    ...
    await withTempFile("b2.ts", "aaa\nbbb\nccc\nddd\n", async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b2.ts", remove_from: a, remove_to: b, replacement_text: "X\nY" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    });
    ...
    await withTempFile("b3.ts", "aaa\nbbb\nccc\nddd\n", async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b3.ts", remove_from: a, remove_to: b, replacement_text: "X\nY\nZ" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b4.ts", "aaa\nbbb\nccc\nddd\n", async ({ cwd, path }) => {
      ...
      const b = readAnchor(r1.text, "│ccc");
      await writeFile(path, "AAA\nbbb\nccc\nddd\n", "utf-8");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b4.ts", remove_from: a, remove_to: b, replacement_text: "X\nY" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    });
    ...
    await withTempFile("b5.ts", "a\nb\nc\nd\ne\n", async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b5.ts", remove_from: a, remove_to: b, replacement_text: "X\nY" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b6.ts", "a\nb\nc\nd\n", async ({ cwd, path }) => {
      ...
      await writeFile(path, "a\nb\nc\nd\n", "utf-8");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b6.ts", remove_from: a, remove_to: b, replacement_text: "X\nY" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b7.ts", Array.from({ length: 9 }, (_, i) => `l${i + 1}`).join("\n"), async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b7.ts", remove_from: a, remove_to: b, replacement_text: "X\nY\nZ\nW\nV" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b8.ts", "aaa\nbbb\nccc\n", async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b8.ts", remove_from: hashes[1]!, remove_to: hashes[1]!, replacement_text: "BBB" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b9.ts", "aaa\nbbb\nccc\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│bbb");
      await writeFile(path, "aaa\nBBB\nccc\n", "utf-8");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b9.ts", remove_from: a, remove_to: a, replacement_text: "X" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b10.ts", "a\nb\nc\nd\n", async ({ cwd, path }) => {
      ...
      await writeFile(path, "a\nb\nb\nd\n", "utf-8");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b10.ts", remove_from: a, remove_to: b, replacement_text: "X\nY\nZ\nW" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b11.ts", "aaa\nbbb\nccc\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│bbb");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b11.ts", remove_from: a, remove_to: a, replacement_text: "bbb" }, ctx);
    ...
    await withTempFile("b12.ts", "a\nb\nc\nd\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│a");
      await writeFile(path, "a\nb\nc\nD\n", "utf-8");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b12.ts", remove_from: a, remove_to: a, replacement_text: "a" }, ctx);
      rec.outcome = e1.ok ? (e1.text.includes("noop") ? "success" : "error") : "rejected";
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    });
    ...
    await withTempFile("b13.ts", "a\nb\nc\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│b");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b13.ts", remove_from: a, remove_to: a, replacement_text: "B" }, ctx);
      ...
      if (plusRow) {
        ...
        const e2 = await call(rec, getTool("replace"), "replace", { path: "b13.ts", remove_from: plusHash, remove_to: plusHash, replacement_text: "B2" }, ctx);
        if (!e2.ok) {
    ...
    await withTempFile("b14.ts", "", async ({ cwd, path }) => {
      ...
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b14.ts", remove_from: emptyHash, remove_to: emptyHash, replacement_text: "first\nsecond" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile(
      ...
      async ({ cwd, path }) => {
        ...
        const e1 = await call(rec, getTool("replace"), "replace", { path: "b15.ts", remove_from: a, remove_to: b, replacement_text: "replacement" }, ctx);
        rec.outcome = e1.ok ? "success" : "rejected";
        rec.code = e1.code;
        rec.finalContent = await readFile(path, "utf-8");
        results.push(rec);
    ...
    await withTempFile("b16.ts", "aaa\nbbb\nccc\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│bbb");
      await call(rec, getTool("replace"), "replace", { path: "b16.ts", remove_from: a, remove_to: a, replacement_text: "BBB" }, ctx);
      const u1 = await call(rec, getTool("undo_last_replace"), "undo", { path: "b16.ts" }, ctx);
    ...
    await withTempFile("b16b.ts", "aaa\nbbb\nccc\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│bbb");
      await call(rec, getTool("replace"), "replace", { path: "b16b.ts", remove_from: a, remove_to: a, replacement_text: "BBB" }, ctx);
      await writeFile(path, "AAA\nBBB\nccc\n", "utf-8");
      const u1 = await call(rec, getTool("undo_last_replace"), "undo", { path: "b16b.ts" }, ctx);
      rec.outcome = u1.ok ? "success" : "rejected";
      rec.code = u1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b17.ts", "a\nb\nc\nd\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│b");
      const b = readAnchor(r1.text, "│c");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b17.ts", remove_from: b, remove_to: a, replacement_text: "X\nY" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
    ...
    await withTempFile("b18.ts", "a\nb\nc\n", async ({ cwd, path }) => {
      ...
      const a = readAnchor(r1.text, "│b");
      const e1 = await call(rec, getTool("replace"), "replace", { path: "b18.ts", remove_from: a, remove_to: a, replacement_text: "a\nX" }, ctx);
      rec.outcome = e1.ok ? "success" : "rejected";
      rec.code = e1.code;
      rec.finalContent = await readFile(path, "utf-8");
      results.push(rec);
