...
describe("edit payload contract", () => {
  it("registers the object-root { path, edits } payload", () => {
    ...
    expect(validator.Check({ path: "sample.ts", edits: [["aB3", "cD4", "new"]] })).toBe(true);
    expect(validator.Check({ path: null, edits: [["aB3", "cD4", "new"]] })).toBe(true);
    expect(
      validator.Check({
        path: "sample.ts",
        edits: [
    ...
    expect(validator.Check({ path: "", edits: [["aB3", "cD4", "new"]] })).toBe(false);
    expect(validator.Check({ path: "sample.ts", edits: [] })).toBe(false);
    expect(validator.Check({ edit: ["sample.ts", ["aB3", "cD4"], "new"] })).toBe(false);
  });
  ...
  it("rejects malformed payloads before mutation", async () => {
    await withTempFile("sample.ts", "aaa\nbbb\n", async ({ cwd, path }) => {
      ...
      await expect(
        tool.execute("e1", { path: "sample.ts", edits: [["bad"]] } as any, undefined, undefined, {
          cwd,
        } as any),
