...
describe("auto-read handler", () => {
  ...
  it("appends the auto-read block after a write following session start", async () => {
      ...
      const result = await handler!(
        {
          ...
          content: [
            {
              type: "text",
              text: "Successfully edited in diff.txt. Added 1 line(s), removed 1 line(s).",
            },
          ],
        },
        { cwd: dir },
      ...
      const result = await handler!(
        {
          ...
          details: {
            ...
            warnings: ['[E_BAD_ANCHOR] stripped "HASH│" prefix from replacement_text line 1.'],
          },
