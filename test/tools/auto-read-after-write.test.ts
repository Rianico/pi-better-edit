...
describe("auto-read after write", () => {
	it("triggers auto-read after a successful write", async () => {
		...
		try {
			...
			expect(writeResult!.content![1]!.text!).toContain(
	...
	it("appends hashline read output after a successful write", async () => {
			...
			const writeResult = await handler!(
				{
					...
					input: { path: "test.txt", content: "hello\nworld\n" },
					content: [
						{ type: "text", text: "Successfully wrote 12 bytes to test.txt" },
					],
					details: undefined,
					isError: false,
				},
	...
	it("returns original write result when auto-read fails", async () => {
		...
		try {
			...
			expect(writeResult).toBeDefined();
			const content = (
				writeResult as { content: Array<{ type: string; text: string }> }
			).content;
			...
			const writeResult = await handler!(
				{
					...
					content: [
						{ type: "text", text: "Successfully wrote 38 bytes to code.ts" },
					],
					details: undefined,
					isError: false,
				},
			...
			const writeResult = await handler!(
				{
					...
					content: [
						{
							type: "text",
							text: "Successfully wrote 1890 bytes to large.txt",
						},
					],
					details: undefined,
					isError: false,
			...
			const writeResult = await handler!(
				{
					...
					content: [
						{ type: "text", text: "Successfully wrote 0 bytes to empty.txt" },
					],
					details: undefined,
					isError: false,
				},
			...
			const editResult = await handler!(
				{
					...
					content: [
						{
							type: "text",
							text: "Successfully edited in edit.txt. Added 1 line(s), removed 1 line(s).",
						},
					],
