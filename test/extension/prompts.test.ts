...
const editPrompt = readFileSync(new URL("../../prompts/edit.md", import.meta.url), "utf-8");
...
const readPrompt = readFileSync(new URL("../../prompts/read.md", import.meta.url), "utf-8");
...
describe("payload-contract prompt generation", () => {
  it("generates prompts from single source with no drift", () => {
    expect(readFileSync(new URL("../../prompts/edit.md", import.meta.url), "utf-8").trim()).toBe(
      EDIT_DESCRIPTION,
    );
    expect(
      readFileSync(new URL("../../prompts/edit-snippet.md", import.meta.url), "utf-8").trim(),
    ).toBe(EDIT_SNIPPET);
