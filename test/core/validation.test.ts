...
describe("valKind", () => {
  it("throws for directory", () => {
    expect(() => valKind({ kind: "directory" }, "test.txt")).toThrow(
      /\[E_UNSUPPORTED_FILE\] Path is a directory: test\.txt\./,
    );
  });
  ...
  it("throws for image file", () => {
    expect(() => valKind({ kind: "image", mimeType: "image/png" }, "test.png")).toThrow(
      /\[E_UNSUPPORTED_FILE\] Path is an image file: test\.png/,
    );
  });
...
describe("valAccess", () => {
  it("throws ENOENT error for missing file", async () => {
    await expect(valAccess("/nonexistent/path.txt", "path.txt")).rejects.toThrow(
      "[E_NOT_FOUND] File not found: path.txt",
    );
  });
  ...
  it.skipIf(process.platform === "win32")("throws EACCES error for unreadable file", async () => {
    ...
    try {
      await expect(valAccess(testFile, "unreadable.txt")).rejects.toThrow(
        "[E_ACCESS] File is not readable: unreadable.txt",
      );
    } finally {
  ...
  it("throws writable error when W_OK requested", async () => {
    ...
    try {
      await expect(
        valAccess(testFile, "readonly.txt", constants.R_OK | constants.W_OK),
