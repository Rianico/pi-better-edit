...
describe("resEdits", () => {
	it("resolves replace with hash_range_incl", () => {
		const edits: HTEdit[] = [
      { hash_range_incl: ["ZZP", "PPW"], content_lines: ["a", "b"] },
		];
	...
	it("resolves a 1-line replace (same anchor)", () => {
		const edits: HTEdit[] = [
      { hash_range_incl: ["MQX", "MQX"], content_lines: ["new"] },
		];
		const resolved = resEdits(edits);
		expect(resolved).toHaveLength(1);
	...
	it("throws on malformed hash_range_incl", () => {
		const edits: HTEdit[] = [
      { hash_range_incl: ["not-valid", "not-valid"], content_lines: ["x"] },
		];
    ...
    const edits = [{ hash_range_incl: ["ZZP", "ZZP"], content_lines: ["x"], extra: true }] as any;
		expect(() => resEdits(edits)).toThrow(
