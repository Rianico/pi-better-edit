...
describe("hash-store — served corrupt row handling", () => {
  async function corruptServed(home: string, path: string, value: string): Promise<void> {
  ...
  it("treats a row with unparseable hashes as an empty record and deletes it", async () => {
    await withTempHome(async (home) => {
      ...
      const remaining = check.prepare("SELECT COUNT(*) AS n FROM served WHERE path = ?").get("/p.ts") as { n: number };
      check.close();
