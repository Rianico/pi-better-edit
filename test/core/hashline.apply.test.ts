...
import { applyEdit, resEdit, type HEdit } from "../../src/hashline";
...
describe("applyEdit — basic operations", () => {
	it("edits a single line", async () => {
		...
		const edit: HEdit = {
			hash_bounds: [
				await makeTag(content, 2, home.testPath),
				await makeTag(content, 2, home.testPath),
			],
			content_lines: ["BBB"],
		};
	...
	it('treats lines:[""] as a blank line for range edits too', async () => {
		...
		const edit: HEdit = {
			hash_bounds: [
				await makeTag(content, 2, home.testPath),
				await makeTag(content, 3, home.testPath),
			],
			content_lines: [""],
		};
		const result = applyEdit(content, edit);
...
describe("applyEdit — noop detection", () => {
	...
	it("rejects deleting an entire non-empty file", async () => {
		...
		const edit: HEdit = {
			hash_bounds: [
				await makeTag(content, 1, home.testPath),
				await makeTag(content, 2, home.testPath),
			],
			content_lines: [],
		};
...
describe("applyEdit — pure edit preserves boundary duplicates (no auto-fix)", () => {
	it("preserves leading duplication verbatim — no stripping", async () => {
	...
	it("preserves trailing duplication verbatim — no stripping", async () => {
		const content = "before\nold one\nold two\nafter";
		const edit: HEdit = {
			hash_bounds: [
				await makeTag(content, 2, home.testPath),
...
describe("applyEdit — lastChangedLine tracking", () => {
	...
	it("tracks lastChangedLine correctly for multi-line delete", async () => {
		...
		const edit: HEdit = {
			hash_bounds: [
				await makeTag(content, 2, home.testPath),
				await makeTag(content, 4, home.testPath),
			],
			content_lines: [],
		};
