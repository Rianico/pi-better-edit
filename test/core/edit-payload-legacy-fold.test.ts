...
describe("legacy payload folding (ADR-0015 shim)", () => {
  it("folds legacy tuples to objects", () => {
    expect(editRequestFrom({ file: "a.ts", edits: [["aB3", "cD4", "new"]] })).toEqual({
      file: "a.ts",
      edits: [ITEM],
    });
  });
  ...
  it("folds the legacy path root key to file", () => {
    expect(editRequestFrom({ path: "a.ts", edits: [["aB3", "cD4", "new"]] })).toEqual({
      file: "a.ts",
      edits: [ITEM],
    });
  });
  ...
  it("prepareEditArguments returns schema-ready folded objects", () => {
    expect(prepareEditArguments({ path: "a.ts", edits: [["aB3", "cD4", "new"]] })).toEqual({
      file: "a.ts",
      edits: [ITEM],
    });
  });
