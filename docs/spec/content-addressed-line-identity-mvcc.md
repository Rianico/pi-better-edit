...
| Target span contains an unleased interior anchor, an interior lease retired / moved, or a never-served **boundary** row | `served-verification.ts` (`verifyRebasedSpan`, both paths) | `[MODEL] [E_STALE_RANGE]` | Echoes current range under `Current range (fresh read):`; model decides from the rows, no retry mandate (ADR-0022) |
| Target span contains unread **interior** lines between the boundaries | — (accepted by `verifyRebasedSpan`, ADR-0024) | none — the edit applies | The applied diff renders the removal's head + tail with an exact omitted count above `DIFF_REMOVED_CAP` rows (ADR-0024) |
...
**Rejection payloads carry `details.cause`.** Every range-family producer emits it as a user-facing diagnosis — never the model remedy, the code alone selects that (`src/mutation-engine/engine.ts:35-47` preserves it onto the failure). As built (`src/hashline/served-verification.ts:36-44`): the `stale` branch reports `retirement` for both its codes (`src/hashline/lease-resolve.ts:216-230`); the leased-span gate reports `retirement` for a terminal interior lease, `never-served` for an interior row the mirror never served and for an anchor holding no lease, and `served-range staleness` for a truncated-away mirror row and for a live lease whose identity moved off its coordinate (`verifyRebasedSpan`); the tombstone checks report `tombstone` (the interior one runs before the general digest clause, which would otherwise subsume it); an unleased anchor reports `never-served` (`lease-resolve.ts:130`), as do a never-served interior (`served-verification.ts:734`) and an unplaceable bound (`:790`); drift, length and hash mismatches report `served-range staleness` (`:356,393,561,748,761`); a content-path mismatch reports `anchor staleness` (`src/hashline/apply.ts:266`). `served span` stays a reserved glossary value with no current producer. No headline interpolates a canon digest: a 32-bit number names nothing the model can act on.

**Amendment — ADR-0024 (interior exposure + applied-diff cap).** Two as-built statements above are amended. (1) The leased-span gate no longer reports `never-served` for an interior row the mirror never served: that row is accepted, so `details.cause: "never-served"` from `verifyRebasedSpan` now means a **boundary** row or an anchor holding no lease. The library-level seam for callers with no lease source (`validateResolvedSpan`, `served-verification.ts:734`) keeps its own never-served interior diagnosis — the change is confined to the leased path. (2) The applied diff no longer renders every removed row: a removal above `DIFF_REMOVED_CAP` (6) rows renders 2 head rows, ` - ... [N lines omitted] ...`, and 2 tail rows, while the render cursor still advances by every omitted row so each surviving row keeps its exact old line number and hash — the refusal side was already capped at `SERVED_ROWS_CAP` (150) rows.

**Rejection headlines name lines, never digests.** `[E_STALE_RANGE]` canon-tier verdicts read `line N in PATH differs from what was served.` — the served and current canon digests stay internal (issue #151). The cause distinguishes the tier, and the fresh-read rows carry the remedy.
