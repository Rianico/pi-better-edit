...
| # | Delta | Seam |
| :- | :--- | :--- |
| **D1** | The `stale` branch emits a **target-lost** rejection under **`[E_TARGET_LOST]`** (D6): no `Current range` heading of either form, no retry hint, and **no `HASH│content` rows**. The message names the previously served position in prose only. The branch serves rows **only** when exactly one bound is stale and the survivor is live *and* unshifted (its rebased coordinate equals its served coordinate — evidence that no shift occurred); then the window is the served coordinates clamped to the file and the code is **`[E_UNVERIFIED_RANGE]`** with the exact heading `Current range (fresh read):`, no retry hint and no mandate — the model decides from those rows. A clamped window that collapses or misses the file fails closed to `[E_TARGET_LOST]`. The disjunction that served `[E_STALE_RANGE]` for any live unshifted bound was the error: a live head with a deleted tail served a window the model never targeted with a blind-retry affordance. | `src/hashline/lease-resolve.ts:175-176,186-209` |
| **D2** | A target-lost rejection performs **no** `recordRejectionServe` upsert — there are no rows to lease — so an accidental retry cannot write. Leasing stays for every window that identifies the model's range (in-place drift, never-served interior, `E_UNVERIFIED_RANGE` fresh reads, `E_BATCH_ABORT`, content-placeable `E_STALE_ANCHOR`). Because no non-leasing serve is introduced, ADR-0016's *"serve … does so through the atomic lease upsert"* stays literally true. As-built, the skip is owned solely by the `servedRows.length === 0` length check — no code branch (see the as-built record below). | `src/mutation-engine/pipeline.ts:478-497`, `src/served-session/session.ts:670,705` |
...
| **D4** | One wording family across `E_STALE_ANCHOR` / `E_STALE_RANGE` / `E_UNVERIFIED_RANGE` / `E_SUSPICIOUS_TEXT`, and anchors are reported **per distinct anchor** (see Appendix C and Revision check). | `src/hashline/resolve.ts:202`, `src/hashline/lease-resolve.ts:103-120`, `src/hashline/served-guard.ts:259` |
...
| Target span contains unread interior lines | `verifyRebasedSpan` / `ServedVerification` | `[E_STALE_RANGE]` | current range | retry | yes |
| **Leased `line_id` retired, range unidentifiable** (deleted, or one bound stale and the other shifted) | `resolveLeasedEdit` (`stale` branch) | `[E_TARGET_LOST]` | **none** — the message names the previously served position in prose | **read and re-target** | **n/a (no rows)** |
| **Leased `line_id` retired, text re-added elsewhere (Probe `P`)** | same | `[E_TARGET_LOST]` | **none** — text at another coordinate is not the model's range | **read and re-target** | **n/a** |
| Leased `line_id` retired **in place**, other bound live and unshifted | same | `[E_UNVERIFIED_RANGE]` | the named window (served coordinates clamped to the file) under `Current range (fresh read):`, no retry hint — the model decides from those rows | decide | yes |
| In-place content/canon mismatch, or tombstoned interior, at the model's coordinates | `ServedVerification` | `[E_STALE_RANGE]` | current range | retry | yes |
...
5. `test/hashline/lease-resolve.test.ts` (extend) — the `stale` branch returns `[E_TARGET_LOST]` with **no** `servedRows` and no retry hint when the range is unidentifiable, and `[E_UNVERIFIED_RANGE]` with the named window as a fresh read under `Current range (fresh read):` (no retry hint) when exactly one bound is stale and the survivor is live and unshifted (D1/D6).
...
9. **Oracle (arch):** `test/arch/rejection-payload-region.test.ts` — every rejection payload's rows are derivable from the submitted anchors' live mapping (`served`/rebased coordinates); a row placed by a content match fails the test (D5, ADR-0018 decision 4). As-built, the oracle is a runtime matrix driven through the public edit seam plus planted-violation negative controls. The `assertLivePayload` helper pins the invariant per case with an `expectedCode`: an `[E_TARGET_LOST]` payload carries zero rows and no `Current range` heading of either form, an `[E_UNVERIFIED_RANGE]` payload carries rows under the exact `Current range (fresh read):` heading with no retry hint, and every other payload carries rows under `Current range:` with a retry hint; each row's hash reproduces the current on-disk hashes exactly and each row's line stays inside the caller-named live window. The file never imports the content lookup, and each window is hardcoded from served-coordinate knowledge, so a content-placed window cannot satisfy the check. Scenario matrix: live lease applies with no rejection; in-place interior retire keeps `[E_STALE_RANGE]` with the served window (lines 1–3); boundary survivor live and unshifted serves `[E_UNVERIFIED_RANGE]` as a fresh read (lines 1–2, leased so deciding from them writes); both bounds stale rejects `[E_TARGET_LOST]`; shifted survivor rejects `[E_TARGET_LOST]` naming the lease-derived served line; collapsed window (served lines 9–10 against a 4-line file) rejects `[E_TARGET_LOST]`; tombstone boundary serves `[E_UNVERIFIED_RANGE]`; interior hole serves `[E_STALE_RANGE]`; re-added text elsewhere (Probe `P`) rejects `[E_TARGET_LOST]` naming the served line 2, never line 4; deleted target rejects `[E_TARGET_LOST]` naming line 3; unleased boundary rejects `[E_STALE_ANCHOR]` with its current window (lines 4–5); duplicate anchor rejects `[E_STALE_ANCHOR]` with both colliding rows; batched call aborts atomically (`[E_BATCH_ABORT]`, later span served) and a stale item keeps its own `[E_TARGET_LOST]` plus the atomicity trailer. Planted-violation evidence: a misplaced-row payload, a rows-carrying target-lost payload, a retry hint on an unverified payload, a stale heading on an unverified payload, and a shifted survivor still serving are constructed and each asserted to fail the check, proving the oracle can fail. Code disjointness (item 10) is enforced per case by the same helper.
10. **Code disjointness (arch):** every `[E_TARGET_LOST]` payload has zero rows and every `[E_STALE_RANGE]` / `[E_UNVERIFIED_RANGE]` payload has at least one; the row-carrying set never overlaps the rowless one (D6).
...
| `src/hashline/served-guard.ts` (new) | `E_SUSPICIOUS_TEXT` gate + `mode: "literal"` escape | **adds a third recovery contract** to D4's wording unification — `Omit the copied anchors from `replace_with` and retry with the same anchors, or declare intent with mode: "literal"` (`served-guard.ts:259`, `:275`). ADR-0009's 2026-09-15 revision freezes the literals `findServedHashEcho` / `E_SUSPICIOUS_TEXT` / `served hash echo` |
| content surface (`src/hashline/resolve.ts`, −63 lines) | shape-based refusal removed | `replace_with` containing anchor-shaped lines whose anchors/canons were never served is now **written verbatim** (measured: `ZZZ│alpha` reached disk, `Successfully edited 1 file(s)`). Intentional — ADR-0009 revision: *"the tool never gates on the shape of a line"*; served-row echoes are still refused. It re-opens the session's `[E_MALFORMED_ANCHOR]` failure class as a fail-open write, so D4 should decide whether the unserved case warrants the non-blocking `[MODEL]` note that the served-prefix tier already uses. `README.md:185` was **not** updated: it still documents `[E_MALFORMED_ANCHOR]` for a `replace_with` holding a `HASH│` prefix, a refusal that no longer exists — the row must be split into anchor-field refusal (`[E_MALFORMED_ANCHOR]`, still live in `parse.ts` and `resolve.ts:340-358`) vs content refusal (`[E_SUSPICIOUS_TEXT]`, served rows only) |
