...
| **D4** | One wording family across `E_STALE_ANCHOR` / `E_STALE_RANGE` / `E_UNSERVED_RANGE` / `E_MALFORM_TEXT`, and anchors are reported **per distinct anchor** (see Appendix C and Revision check). | `src/hashline/resolve.ts:202`, `src/hashline/lease-resolve.ts:103-120`, `src/hashline/served-guard.ts:259` |
