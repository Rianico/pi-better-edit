...
2. As a model whose edit range contains a line that changed externally since it was served, I want a hard `[E_STALE_RANGE]` rejection naming the first offending line, so that I never apply an edit over content I haven't seen.
3. As a model rejected with `[E_STALE_RANGE]`, I want the current range served as fresh `HASH│content` rows, so that I can retry immediately without calling `read`.
4. As a model retrying after a rejection, I want the served rows from the rejection to count as serves, so that the retry's range verifies cleanly and reject-and-serve terminates instead of looping.
5. As a model whose range interior contains lines I was never shown (paged reads, disjoint diff hunks, truncated auto-read previews), I want `[E_STALE_RANGE]` plus the current range, so that I never edit lines blind.
...
16. As a model, I want the drift notice to fire once per drift episode, so that subsequent edits do not re-serve the same drifted rows.
...
2. **Serve surfaces** — the served record is written wherever a row is delivered to the model's context: `read` output rows; auto-read-after-`write` preview rows; post-edit diff rows for `replace` and `undo` (delivered via the `tool_result` handler when auto-read is enabled); and error/rejection feedback rows (`[E_STALE_ANCHOR]` context, `[E_STALE_RANGE]`/`[E_UNVERIFIED_RANGE]` range serves). Preview computation (`noPersist`) never writes served state. With auto-read disabled, diff rows are not delivered and therefore not recorded.
3. **Span verification** — a pure verification step in the `replace` apply path, executed on the final resolved range after all autocorrection (reversed-range swap, prefix stripping, boundary-duplicate stripping) and before splicing: locate the served positions of the two boundary hashes; require a contiguous fully-served span between them; compare that served span line-for-line against the current resolved span. Any gap (never-served entry) → `[E_STALE_RANGE]`; any hash inequality → `[E_STALE_RANGE]`; served-side ambiguity (a boundary hash served at multiple positions) or a missing served position → `[E_UNVERIFIED_RANGE]` fresh-read rejection. Single-line ranges have no interior and pass trivially.
4. **Error contract** — two serving codes: `[E_STALE_RANGE]` (served-but-changed, or never-served interior — the remedy is identical) and `[E_UNVERIFIED_RANGE]` (unplaceable bound — a fresh read to decide from). `[E_STALE_RANGE]` names the first offending line and serves the current range as `HASH│content` rows with a retry hint; `[E_UNVERIFIED_RANGE]` carries one general headline clause and serves the named window under `Current range (fresh read):` with no retry hint. Rows are capped at ~150 lines with a pagination hint past the cap. All served rows are recorded as serves — this is what makes retry-without-read terminate and what makes deciding-from-the-fresh-read writable.
5. **Drift notice** — after a successful `replace` (applied or noop, not `undo`), when served territory outside the resolved range has drifted, append an informational section with the current content of the drifted lines plus the current-file lines immediately around them (capped like `read` output); all shown rows count as serves. Fires once per drift episode: reported drifted hashes are tracked; a later `replace` finding only already-reported drift emits a one-line pointer instead of re-serving rows, until a `read` re-serves the lines. Drift inside the range is the check's reject path — the two never overlap. The drift scan compares the pre-edit served record (outside the range) against the post-edit file, and must run before the diff-driven served-record update (or against a snapshot of the pre-update record).
...
- Design authority: `docs/adr/0001-served-state-range-verification.md` (status: **accepted**) and the `CONTEXT.md` glossary. Use its vocabulary throughout: *serve, served state, span, served span, served-range staleness, never-served, reject-and-serve, drift, drift notice, model–tool boundary, anchor philosophy*.
