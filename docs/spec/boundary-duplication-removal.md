...
- **Keep served hash echo/bare-prefix only**: `test/core/hashline-apply-internals.test.ts`, `test/core/hashline.apply.test.ts` — drop `autoFixes` assertions, keep `E_MALFORMED_ANCHOR`/`E_MALFORM_TEXT` paths.
