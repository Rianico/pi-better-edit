...
- **Keep served hash echo/bare-prefix only**: `test/core/hashline-apply-internals.test.ts`, `test/core/hashline.apply.test.ts` — drop `autoFixes` assertions, keep `E_BAD_ANCHOR`/`E_SERVED_ECHO` paths.
