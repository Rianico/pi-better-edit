...
4. **Cheap diagnostic cleanups:** dedupe counts (6), single served block (7), strip the doubled tag (8), optional all-digit-anchor hint (9). The `README.md:185` `[E_BAD_ANCHOR]` split (10) is **retired as satisfied** — `git show d528c0f -- README.md` scoped the row to anchor fields (`Anchor fields only — replace_with is never refused for its shape; a replacement that reproduces a served row is [E_SERVED_ECHO]`), the current `README.md` carries that row plus a separate `[E_SERVED_ECHO]` row, and the `HASH│` mention is a diff-preview marker inside the anchor-field enumeration. No rows duplicated.
