...
| **Pre-existing** | 6 | allocation function (5), duplicate count/plural (6), batch-abort repetition (7), doubled `[MODEL]` tag (8), digit anchors (9), `E_MALFORMED_ANCHOR` on bash output (10) — note that the **content** arm of `E_MALFORMED_ANCHOR` is gone on `ba7c8d2` (the code now covers anchor fields only) and `README.md:185` still documents the removed refusal |
...
| 10 | `E_MALFORMED_ANCHOR` on bash/`sed` output | **changed, not fixed** | shape-based refusal was removed from the content surface on purpose (ADR-0009 revision 2026-09-15, *"the tool never gates on the shape of a line"*): `replace_with` holding `ZZZ│alpha` (never served) is now **written verbatim** and reported `Successfully edited`; a *served* row echo is refused as `[E_SUSPICIOUS_TEXT]` with a `mode: "literal"` escape |
...
New surface to absorb into the wording work: `[E_SUSPICIOUS_TEXT]` + `mode: "literal"` is a **third** rejection contract alongside "re-read" and "retry with these anchors" (frozen literals per ADR-0009 revision; see the patch spec's Revision check). Findings 1, 3, 6, 7 and 9 were each re-measured with a scratch repro that was run and then deleted — they are evidence for this handoff, not landed regression tests; nothing in the upstream suite regressed.
...
| 7 | `toolchain-wiki/subskills/worktrunk/SKILL.md` | `E_MALFORMED_ANCHOR` | `sed -n`/`grep` output (`wt list --format=json \| jq …`) | bash output used as anchors (no anchors there); pre-existing validation |
...
4. **Cheap diagnostic cleanups:** dedupe counts (6), single served block (7), strip the doubled tag (8), optional all-digit-anchor hint (9). The `README.md:185` `[E_MALFORMED_ANCHOR]` split (10) is **retired as satisfied** — `git show d528c0f -- README.md` scoped the row to anchor fields (`Anchor fields only — replace_with is never refused for its shape; a replacement that reproduces a served row is [E_SUSPICIOUS_TEXT]`), the current `README.md` carries that row plus a separate `[E_SUSPICIOUS_TEXT]` row, and the `HASH│` mention is a diff-preview marker inside the anchor-field enumeration. No rows duplicated.
