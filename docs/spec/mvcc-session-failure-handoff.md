...
New surface to absorb into the wording work: `[E_MALFORM_TEXT]` + `mode: "literal"` is a **third** rejection contract alongside "re-read" and "retry with these anchors" (frozen literals per ADR-0009 revision; see the patch spec's Revision check). Findings 1, 3, 6, 7 and 9 were each re-measured with a scratch repro that was run and then deleted — they are evidence for this handoff, not landed regression tests; nothing in the upstream suite regressed.
