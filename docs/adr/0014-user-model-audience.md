...
- **Message pairing:** `E_STALE_RANGE`/`E_UNSERVED_RANGE` throws emit `[MODEL] [E_*] …` + second line `[USER] Current range below is fresh — retry with these anchors, no read needed` (dimmed in TUI) + `Current range:` serve block with `servedRows` (`reject-and-serve`). `E_STALE_ANCHOR` includes `Current context around resolved anchor`.
