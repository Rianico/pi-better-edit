...
accepted — amends [ADR-0016](0016-content-addressed-line-identity-supersedes-healing.md)'s Consequences recovery clause; amended by [ADR-0020](0020-unverified-range-replaces-unserved-range-boundary-rule-for-retired-identities.md) (boundary rule, decisions 1–2 and 4). ADR-0016's Decision is **not** superseded: no non-leasing serve is introduced, so `serve` remains the only operation that presents an anchor and it still does so through the atomic lease upsert.
...
1. The `stale` branch serves rows **only** when exactly one bound is stale and the surviving bound is live *and* its rebased coordinate equals its served coordinate — evidence that no shift occurred. The window is then the served coordinates clamped to the file, the code is **`[E_UNVERIFIED_RANGE]`** with the exact heading `Current range (fresh read):`, one general headline clause, no retry hint and no mandate, and the rows are leased as today. (Amended by ADR-0020: the original disjunction — any live unshifted bound serving `[E_STALE_RANGE]` — was the error. A live head with a deleted tail served a window the model never targeted with a blind-retry affordance.)
2. Otherwise the rejection is emitted as **`[E_TARGET_LOST]`** and carries **no rows**. The message names the previously served position and states that the anchors describe a version of this file that no longer exists, so a re-read is required. The row-carrying codes stay disjoint from it by payload shape — `[E_STALE_RANGE]`, `[E_UNVERIFIED_RANGE]` and `[E_STALE_ANCHOR]` always render rows, `[E_TARGET_LOST]` never does — so a model can choose its remedy from the code alone.
...
4. **Verification oracle:** an architecture test asserts that every rejection payload's rows are derivable from the submitted anchors' live mapping (served / rebased coordinates) — never from a content match. This is the guard that keeps variant 2 from re-entering through a path nobody has thought of. As-built (`test/arch/rejection-payload-region.test.ts`, `26c3a34`, amended by ADR-0020): a runtime matrix driven through the public edit seam plus planted-violation negative controls. The `assertLivePayload` helper pins the derivability rule per case with an `expectedCode` — an `[E_TARGET_LOST]` payload carries zero rows and no `Current range` heading of either form, an `[E_UNVERIFIED_RANGE]` payload carries rows under the exact `Current range (fresh read):` heading with no retry hint, and every other payload carries rows under `Current range:` with a retry hint; each row's hash reproduces the current on-disk hashes exactly and each row's line stays inside the caller-named live window. The test file never imports the content lookup and each window is hardcoded from served-coordinate knowledge, so a content-placed window cannot satisfy the check. The matrix covers: live lease applies; boundary survivor live and unshifted (`[E_UNVERIFIED_RANGE]` fresh read, leased so deciding from it writes); both bounds stale, shifted survivor, and collapsed window (`[E_TARGET_LOST]`); tombstone boundary (`[E_UNVERIFIED_RANGE]`); interior hole (`[E_STALE_RANGE]`); in-place interior retire (`[E_STALE_RANGE]`, served window); re-added text elsewhere (Probe `P`, `[E_TARGET_LOST]` naming the served coordinate); deleted target (`[E_TARGET_LOST]`); unleased boundary (`[E_STALE_ANCHOR]`); duplicate anchor; batched atomic abort with a stale item keeping its own code. Planted-violation evidence: a misplaced-row payload, a rows-carrying target-lost payload, a retry hint on an unverified payload, a stale heading on an unverified payload, and a shifted survivor still serving are constructed and each asserted to fail the check.

Unchanged: the fast path, the dynamic rebase, in-place `E_STALE_RANGE` for torn or inserted spans (range identifiable, current content served), the content-placeable `E_STALE_ANCHOR` self-heal for an unleased anchor, and the `E_SERVED_ECHO` / `mode: "literal"` surface. Retired by ADR-0020: `E_UNSERVED_RANGE` (interior hole → `E_STALE_RANGE`, boundary producer → `E_UNVERIFIED_RANGE`).

### Considered Options

- **Serve nothing for the whole `stale` branch** — rejected. It also removes the case where the range *is* identifiable and the served rows are the model's question refreshed; it contradicts ADR-0001's mechanism and inverts `test/integration/served-range-verification.test.ts:250` (`records [E_UNVERIFIED_RANGE] fresh-read rows as serves for edits over that territory`, as renamed by ADR-0020). Safety was never the same thing as refusing to serve.
...
- **Tests:** `served-range-verification.test.ts:250` must keep passing (boundary survivor live and unshifted, now `[E_UNVERIFIED_RANGE]`); new tests pin (i) a deleted target → `[E_TARGET_LOST]` with no rows + re-read, (ii) Probe P → `[E_TARGET_LOST]`, no line-4 window, (iii) the derivable-rows oracle of decision 4, and (iv) code disjointness (`[E_TARGET_LOST]` ⇒ 0 rows, `[E_STALE_RANGE]`/`[E_UNVERIFIED_RANGE]` ⇒ ≥ 1 row with their respective headings).
- **New public literal:** `[E_TARGET_LOST]` must be added to `README.md`'s error table, `CONTEXT.md`, and the prompts; downstream consumers keyed on error codes (e.g. `dsh-better-edit`) see one additional code and no changed meaning for `[E_STALE_RANGE]`. The full producer audit lives in the patch spec's Appendix E. ADR-0020 adds `[E_UNVERIFIED_RANGE]` and retires `[E_UNSERVED_RANGE]` under the same table.
