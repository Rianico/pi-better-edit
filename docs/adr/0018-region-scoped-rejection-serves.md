...
4. **Verification oracle:** an architecture test asserts that every rejection payload's rows are derivable from the submitted anchors' live mapping (served / rebased coordinates) — never from a content match. This is the guard that keeps variant 2 from re-entering through a path nobody has thought of. As-built (`test/arch/rejection-payload-region.test.ts`, `26c3a34`): a runtime matrix driven through the public edit seam plus a planted-violation negative control. The `assertLivePayload` helper pins the derivability rule per case — an `[E_TARGET_LOST]` payload carries zero rows and no `Current range:` heading, while every other payload carries rows under that heading with each row's hash reproducing the current on-disk hashes exactly and each row's line inside the caller-named live window. The test file never imports the content lookup and each window is hardcoded from served-coordinate knowledge, so a content-placed window cannot satisfy the check. The matrix covers: live lease applies; in-place retire (`[E_STALE_RANGE]`, served window); re-added text elsewhere (Probe `P`, `[E_TARGET_LOST]` naming the served coordinate); deleted target (`[E_TARGET_LOST]`); interior gap (`[E_UNSERVED_RANGE]`); unleased boundary (`[E_STALE_ANCHOR]`); duplicate anchor; batched atomic abort with a stale item keeping its own code. Planted-violation evidence: a misplaced-row payload and a rows-carrying target-lost payload are constructed and asserted to fail the check.
