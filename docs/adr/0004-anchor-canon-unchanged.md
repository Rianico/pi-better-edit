...
After measuring that prettier-style formatting rotates anchors for ~93–98% of the lines it touches (the motivation for ADR-0003), we re-examined whether the anchor should strip ASCII whitespace `[ \t\r\n]`. We decided **no: the anchor canon stays as-is** (`canon()` = strip `\r` + trim trailing whitespace). The anchor-stripping half of the two-signal design is rejected; reject-and-serve already handles formatting churn correctly (one extra round-trip, zero correctness risk), and the measured win only helps the *external-formatter-between-read-and-edit* case, not the common agent-driven flow where writes/edits re-serve fresh anchors anyway.
