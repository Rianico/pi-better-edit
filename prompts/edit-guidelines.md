- edit: use exactly `[path, [remove_from, remove_to], replacement_text]`; `path` is a non-empty string or `null`, the range is exactly two inclusive anchors, and replacement_text is complete (use `""` to delete).
