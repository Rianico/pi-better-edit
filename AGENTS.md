...
- `computeLineHash(idx, line)` is a backward-compat single-line helper. It is not used by production code; it exists for test use where a single-line hash is needed without the full file context.
