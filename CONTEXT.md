...
The condition where the `served range` (span between anchors) cannot be reconciled with served state: interior `served span` vs `current span` mismatch (`hash`/`canon`/`tombstone`/`len`), a served interior row whose lease is retired or whose `line_id` moved off its coordinate, a boundary row the mirror never served, or a mirror row a serve truncated away while its lease still lives (issue #151). Reported as `[E_STALE_RANGE]` — retired: `[E_UNSERVED_RANGE]`, see ADR-0020. An **unread interior** line between two leased boundaries is not staleness and does not fail closed (ADR-0024): it carries no identity to verify, and the boundaries plus the window-length gate already pin the span. Every `[E_STALE_RANGE]` does `reject-and-serve`: the current range is served under the exact heading `Current range (fresh read):` with no retry hint and no mandate — the rows prove served state and disk disagree, not that the same anchors are the right retry, so the model decides from them instead of retrying blind (ADR-0022).
_Avoid_: range staleness (use `served range` for span)

**never-served**:
A line with no entry in the served record — the model was never served that line. A user-facing diagnosis carried as `details.cause` on the rejection, never the model remedy (the code alone selects the retry). It fails closed only where there is no identity left to verify: a **boundary** row of a leased span (one of the two anchors the model named) reports `[E_STALE_RANGE]`; an unplaceable boundary (one bound stale, survivor live and unshifted) reports `[E_UNVERIFIED_RANGE]`; an anchor with no lease at all reports `[E_UNKNOWN_ANCHOR]` (no lease in any file) or `[E_FOREIGN_ANCHOR]` (leased for another file) — both rowless without remedy. An **unread interior** row between two leased boundaries is accepted instead, because it carries no identity to verify (ADR-0024). `[E_STALE_ANCHOR]` names a served anchor whose identity is dead (rows served, retry with them).
...
**reject-and-serve**:
The staleness policy for a range-matched rejection: reject the edit and return the current range as fresh `HASH│content` rows, which themselves count as serves. The range-family codes (`[E_STALE_RANGE]`, `[E_UNVERIFIED_RANGE]`) serve those rows under `Current range (fresh read):` with no retry hint and no mandate — the model decides from them instead of retrying blind; `[E_STALE_ANCHOR]` keeps the `Current range:` retry hint, because there the rows themselves are the retry. A rejection whose range cannot be identified carries no rows (see `target-lost rejection`).
...
**tombstone**:
The per-session (`sessionKey`, `path`) set of hashes freed since the last full `read` — `served.retired` in `src/served-session/session.ts:815-830`. Allocation (`HashIdentity`) treats `used = bitset(oldHashes) ∪ bitset(tombstone)` so a freed anchor never re-binds for the session (`src/hashline/hash-identity.ts:186-194`, `:319-331`, plumbed via `src/hashline/hash.ts:47-65`). ADR-0017 retains it only as the hash-allocation guard, never as a second identity authority — and its verification job is a signal, not a lease state, so the `retirement` (`retired_at`) term above still owns lease-terminal state. Its second live job is that verification signal, now confined to the library-level seam: `src/hashline/served-verification.ts` rejects a tombstoned boundary hash as `[E_STALE_ANCHOR]` via `throwStaleForTombstone` (rows served with the retry hint, `details.cause: "tombstone"`) and a tombstoned interior as `[E_STALE_RANGE]` (rows served as a fresh read, same cause) for a caller that presents a served mirror and **no** lease source; a leased session edit resolves the same span by lease retirement (`details.cause: "retirement"`), which supersedes it (issue #151). ADR-0020 decision 2 prescribed the fresh-read heading for the boundary check; as-built `0bc2b65` serves `[E_STALE_ANCHOR]` rows instead (see ADR-0021 decision 3). Cleared on `full read` (`isFullRead`), kept on `partial`/`truncated`, pruned with `served` via `SERVED_TTL_MS`. Prevents `S@3 reborn @3` whole-span stale success.
...
**canon digest**:
`String(xxh32(canon(line)))` — the opaque canon identity `line_lineage.canon_hash` and `served_leases.canon_hash` store and compare, produced by the single `canonDigest` definition in `src/hashline/hash-identity.ts`. Canon evidence is always a digest: the hash-allocation lineage writer, the served-row evidence scans (`findServedHashEcho`, `findServedPrefixMismatches`), the drift rotated-survivor check and the library-level `verifyServedRange` tier all compare digests derived on demand from the lease that carries them, so the text a digest came from is never persisted (issue #151).
