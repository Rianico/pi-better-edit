...
const engines = engineArg ? [engineArg.slice("--engine=".length)] : ["local", "omp"];
...
function parseRun(log) {
  ...
  return {
    ...
    stopReason: final?.messages?.at(-1)?.usage ? final.messages.at(-1).stopReason : undefined,
  };
}
...
function prepareOmpScratch() {
  ...
  execFileSync(
    "npm",
    ["install", "--silent", "--no-package-lock", "@oh-my-pi/hashline@17.3.5", "bun"],
...
function runEngine(engine) {
  ...
  try {
    ...
    if (runtime?.scratch && !keepScratch) rmSync(runtime.scratch, { recursive: true, force: true });
